...
runPnpmOrExit(["build", ...passthroughArgs], env, tempDocsDir);

console.log(`Lux docs build completed from temp copy: ${tempDocsDir}`);
console.log(`Build output: ${resolve(tempDocsDir, ".svelte-kit/output")}`);
console.log(`Cloudflare output: ${resolve(tempDocsDir, ".svelte-kit/cloudflare")}`);

if (cleanTemp) {
  console.log(`Temp base was reset before build: ${tempBaseDir}`);
}
