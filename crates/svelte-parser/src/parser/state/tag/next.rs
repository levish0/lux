...
pub fn next<'a>(parser: &mut Parser<'a>) {
...
fn next_if<'a>(parser: &mut Parser<'a>, start: usize) {
    if !parser.eat("else") {
        if !parser.loose {
            parser.error(
    ...
    if parser.eat("if") {
        ...
        while elseif_start > 0
            && parser.template.as_bytes()[elseif_start] != b'{'
        {
            elseif_start -= 1;
        }
...
fn next_each<'a>(parser: &mut Parser<'a>, start: usize) {
    if !parser.eat("else") {
        if !parser.loose {
            parser.error(
                ErrorKind::ExpectedToken,
...
fn next_await<'a>(parser: &mut Parser<'a>, start: usize) {
