...
impl BenchContext {
    ...
    fn run_svelte_phase(&self, phase: &str, iterations: u64) -> Duration {
        ...
        let output = Command::new(npx_executable())
            ...
            .output()
            .unwrap_or_else(|err| panic!("failed to run npx for phase {phase}: {err}"));
...
fn bench_analyze_pipeline(c: &mut Criterion) {
    ...
    group.bench_function("lux_parse_and_analyze", |b| {
        b.iter(|| {
            ...
            criterion::black_box(tables.diagnostics.len());
        });
    });

    group.bench_function("svelte_compile_generate_false_npx", |b| {
