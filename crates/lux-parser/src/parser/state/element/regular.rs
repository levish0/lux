...
pub fn parse_regular_element<'a>(
    ...
    let shadowroot_template = name == "template" && has_shadowrootmode_attribute(&attributes);
    let (fragment, end) = with_shadowroot_template(input, shadowroot_template, |input| {
        parse_element_body(input, name)
    })?;
