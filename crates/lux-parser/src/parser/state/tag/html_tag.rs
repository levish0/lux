...
use lux_ast::template::tag::HtmlTag;
use winnow::Result;
use winnow::prelude::*;
...
/// Assumes `{` already consumed. Starts at `@html`.
pub fn parse_html_tag<'a>(input: &mut Input<'a>, start: usize) -> Result<FragmentNode<'a>> {
    literal("@html").parse_next(input)?;
