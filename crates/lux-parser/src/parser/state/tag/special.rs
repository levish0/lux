...
use lux_ast::metadata::{ExpressionNodeMetadata, RenderTagMetadata};
use lux_ast::node::FragmentNode;
use lux_ast::span::Span;
use lux_ast::tags::{ConstTag, DebugTag, HtmlTag, RenderTag};
...
pub fn special(parser: &mut Parser) -> Result<(), ParseError> {
    ...
    if parser.eat("const") {
        ...
        // Validate: SequenceExpression without parentheses is invalid
        // `{@const a = (b, c)}` is allowed but `{@const a = b, c = d}` is not
        // Reference: tag.js:690-696
        if let Expression::SequenceExpression(seq) = &init {
            let init_start = seq.span.start as usize;
            let slice = &parser.template[expression_start..init_start];
            if !slice.contains('(') && !parser.loose {
                return Err(parser.error(
                    ErrorKind::ConstTagInvalidExpression,
                    init_start,
                    "Cannot use a sequence expression in {@const ...}".to_string(),
                ));
            }
        }
